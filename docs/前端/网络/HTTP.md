...
HTTP (The HyperText Transfer Protocol，超文本传输协议) 是用于在 [Web](https://developer.mozilla.org/zh-CN/docs/Glossary/World_Wide_Web) 上传输超媒体文件的底层 [协议](https://developer.mozilla.org/zh-CN/docs/Glossary/Protocol) ，最典型场景的是在浏览器和服务器之间传递数据，以供人们浏览。现行的 HTTP 标准的版本是 [HTTP/2](https://developer.mozilla.org/zh-CN/docs/Glossary/HTTP_2)。

“http://”称为“schema”，是 [URI](https://developer.mozilla.org/zh-CN/docs/Glossary/URI) 的组成部分，一般位于网络地址的开头。以“[https://developer.mozilla.org”为例，该地址说明请求文档时使用](https://developer.mozilla.xn--org%2C-ib7an391ab4byc839fue412ncdb4ih20axtprv8ad14f1ha4b/) HTTP 协议；这里的 https 代指 HTTP 协议的安全版本，即 [SSL](https://developer.mozilla.org/zh-CN/docs/Glossary/SSL) （或称 TLS）

HTTP 是基于文本的 (所有的通信都以纯文本的形式进行) 以及无状态的 (当前通信状态不会发现以前的通信状态)，该特性极大方便了在 www 上浏览网页的人。除此之外，HTTP 也可以用于构建服务器之间交互的 [REST](https://developer.mozilla.org/zh-CN/docs/Glossary/REST) web 服务，以及使得网站内容更加动态化的 [AJAX](https://developer.mozilla.org/zh-CN/docs/Glossary/AJAX) 请求
...
<img src="./assets/fetching_a_page.png" alt="图片加载失败" style="zoom: 100%;" />

客户端和服务端通过交换各自的消息（与数据流正好相反）进行交互。由像浏览器这样的客户端发出的消息叫做 request，被服务端响应的消息叫做 response。

<img src="./assets/http-layers.png" alt="图片加载失败" style="zoom: 50%;" />
...
<img src="./assets/client-server-chain.png" alt="图片加载失败" style="zoom: 50%;" />
...
虽然 HTTP/2 协议将 HTTP 消息封装到了帧（frames）中，HTTP 大体上还是被设计得简单易读。HTTP 报文能够被人读懂，还允许简单测试，降低了门槛，对新人很友好。
...
- _开放同源限制_ 为了防止网络窥听和其它隐私泄漏，浏览器强制对 Web 网站做了分割限制。只有来自于**相同来源**的网页才能够获取网站的全部信息。这样的限制有时反而成了负担，HTTP 可以通过修改头部来开放这样的限制，因此 Web 文档可以是由不同域下的信息拼接成的（某些情况下，这样做还有安全因素考虑）。
- _认证_ 一些页面能够被保护起来，仅让特定的用户进行访问。基本的认证功能可以直接通过 HTTP 提供，使用[`Authenticate`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Authenticate 'This is a link to an unwritten page')相似的头部即可，或用 HTTP Cookies 来设置指定的会话。
- _[代理和隧道](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Proxy_servers_and_tunneling)_ 通常情况下，服务器和/或客户端是处于内网的，对外网隐藏真实 IP 地址。因此 HTTP 请求就要通过代理越过这个网络屏障。但并非所有的代理都是 HTTP 代理。例如，SOCKS 协议的代理就运作在更底层，一些像 FTP 这样的协议也能够被它们处理。
- _会话_ 使用 HTTP Cookies 允许你用一个服务端的状态发起请求，这就创建了会话。虽然基本的 HTTP 是无状态协议。这很有用，不仅是因为这能应用到像购物车这样的电商业务上，更是因为这使得任何网站都能轻松为用户定制展示内容了。
...
- HTTP 协议版本号。
- 为服务端表达其他信息的可选头部[headers](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers)。
...
<img src="./assets/http_response.png" alt="图片加载失败" style="zoom: 67%;" />

响应报文包含了下面的元素：

- HTTP 协议版本号。
- 一个状态码（[status code](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Status)），来告知对应请求执行成功或失败，以及失败的原因。
- 一个状态信息，这个信息是非权威的状态码描述信息，可以由服务端自行设定。
- HTTP [headers](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers)，与请求头部类似。
...
另一方面，如果 [TLS](https://developer.mozilla.org/zh-CN/docs/Glossary/TLS) 桥接代理通过在 PC 上安装来自组织管理的 [CA (en-US)](https://developer.mozilla.org/en-US/docs/Glossary/Certificate_authority 'Currently only available in English (US)') 证书，以中间人方式解密所有通信，并执行访问控制等，则可以查看响应的内容并将其缓存。但是，由于[证书透明度（certificate transparency）](https://developer.mozilla.org/zh-CN/docs/Web/Security/Certificate_Transparency)在最近几年变得很普遍，并且一些浏览器只允许使用证书签署时间戳（signed certificate timestamp）颁发的证书，因此这种方法需要应用于企业策略。在这样的受控环境中，无需担心代理缓存“已过时且未更新”。
...
例如，Varnish Cache 使用 VCL（Varnish Configuration Language，一种 [DSL (en-US)](https://developer.mozilla.org/en-US/docs/Glossary/DSL/Domain_specific_language 'Currently only available in English (US)')）逻辑来处理缓存存储，而 service worker 结合缓存 API 允许你在 JavaScript 中创建该逻辑。
...
<img src="./assets/type-of-cache.png" alt="图片加载失败" style="zoom: 100%;" />
...
确定响应何时是 fresh 的和何时是 stale 的标准是 **age** 。在 HTTP 中，age 是自响应生成以来经过的时间。这类似于其他缓存机制中的 [TTL (en-US)](https://developer.mozilla.org/en-US/docs/Glossary/TTL 'Currently only available in English (US)')。
...
<img src="./assets/keyed-with-url.png" alt="图片加载失败" style="zoom: 100%;" />
...
<img src="./assets/keyed-with-url-and-language.png" alt="图片加载失败" style="zoom: 100%;" />
...
收到该响应后，客户端将存储的陈旧响应恢复为新鲜的，并可以在剩余的 1 小时内重复使用它。服务器可以从操作系统的文件系统中获取修改时间，这对于提供静态文件的情况来说是比较容易做到的。但是，也存在一些问题；例如，时间格式复杂且难以解析，分布式服务器难以同步文件更新时间。
...
<img src="./assets/request-collapse.png" alt="图片加载失败" style="zoom: 50%;" />
...
通过这种设计，JavaScript 和 CSS 资源都可以被缓存很长时间。那么 `max-age` 应该设置多长时间呢？
...
有关更多信息，请参阅 CDN 文档，并查阅 [service worker 文档](https://developer.mozilla.org/zh-CN/docs/Web/API/Service_Worker_API)。
